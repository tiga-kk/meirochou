...
| Task | 内容 | 状態 | 依存 |
|---|---|---|---|
| 1 | サークルスペース表記の正規化 | **次に着手** | なし |
| 2 | カタログ拡張→GAS POST経路の診断 | 未着手 | Task 1 |
| 3 | 購入済みピン非表示と候補表示の分離 | 未着手 | なし |
| 4 | 地図ドラッグ遅延の計測と最小改善 | 未着手 | Task 3 |
| 5 | 現在経路の方向表示強化 | 未着手 | Task 3 |
| 6 | 目的地カタログのモバイルレイアウト修正 | 未着手 | Task 3 |
| 7 | Gallery購入時の退出表示と完全Undo | 未着手 | Task 3 |
| 8 | 実機受入・回帰検証・終了判定 | 未着手 | Task 1〜7 |

複数Taskが着手可能な場合、WIPがあればそれを優先し、なければ番号が最小のTaskを選ぶ。したがって初期状態ではTask 1が一意の次Taskとなる。
