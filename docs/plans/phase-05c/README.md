...
**Goal:** C108の各地図で、排他的なサークル状態、任意始点、到着確認、距離行列、時間制限付きTOPTW、進捗・取消、地図切替、再読込復帰を一貫したモバイルUIとして提供する。

**Architecture:** 永続サークル状態と一時的なナビゲーション状態を分離する。距離行列と最適化はmain thread外で実行し、現在案内中の1区間を固定したまま残り順路だけを改善する。各地図は独立した問題として扱い、地図へ戻ったときは距離行列と以前の解を再利用するが、始点を設定し直す。

**Tech Stack:** TypeScript strict、Lit、LocalStorage repository、Web Worker、weighted 4-neighbor Dijkstra、Python参照実装に対応するTypeScript TOPTW adapter、Vitest、Playwright。
...
- 距離行列はLocalStorageへrepository経由で保存する。
...
- TOPTWのdefault時間は15秒、設定値は5、15、30、60秒。
- TOPTWは現在の1区間を固定し、途中結果を利用できる。
- Python参照実装、Python runtime、元地図、外部情報providerをWebリポジトリへ追加しない。
...
- Python版TOPTW参照実装がGit管理外の指定場所に配置されている。
- TypeScript移植に必要な小規模fixtureを参照実装から作成できる。
...
| 5 | `task-05-distance-matrix-worker.md` | 距離行列、Worker、progress、cancel、LocalStorage cache |
| 6 | `task-06-toptw-adapter-and-worker.md` | Python対応fixture、TypeScript TOPTW、時間制限、warm start |
...
- TOPTWが5/15/30/60秒設定、cancel、warm start、現在区間固定に対応する。
