...
Task 10後に実装する。Task 12とTask 17がzoom transform通知を再利用できるようにする。
...
- `tests/route-overlay-contract.test.ts`
- `tests/e2e/webapp.spec.ts`
- `tests/gesture-zoom-controller.test.ts`（既存する場合）
...
4. `DomRouteMapView`でzoom scaleからcurrent/candidate共通の画面上stroke幅を計算し、CSS custom property等でoverlayへ渡す。
5. scale=1付近では現行の視認性を維持し、scaleが増えるほど線幅を単調に小さくする。4px前後など読める下限を設ける。
6. zoom変更は描画styleだけを更新し、route points、route planning、SVG生成回数を増やさない。
7. moving cueの線幅もbase pathとの比率が不自然にならない範囲で同じscale情報へ追従させる。Task 12のアニメーション診断自体はこのTaskへ混ぜない。
...
- zoom callbackはpan/zoom/reset/setTransformで最新stateを通知する。
