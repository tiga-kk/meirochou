...
Phase 6を本番実機で使って判明した削除、通信状態表示、地図viewer、Gallery gesture、距離・方向表示の問題を修正する。

## 構成方針

Phase 5D/6で確立したfeature境界を維持し、既存Use Case / Session / Controller / Viewを拡張する。地図は実画像比率を持つstageをviewport内でtransformし、animationはCSS/SVGへ限定する。

使用技術はTypeScript/JavaScript、Lit、CSS、SVG、Pointer Events、Vitest、Playwright Chromiumとする。
...
- scale根拠不足があっても、Task 5のS/G・route flow・`physicalPixelLength`など独立部分まで停止しない。
...
| 6 | Phase 6.1全体をE2E/visual/performance観点で再検証 | Task 1〜5の必須部分完了後 |

Task 1/2は管理系、Task 3〜5は会場操作系である。実装担当は一度に1 Taskだけ扱い、各Taskを独立commitとしてレビュー可能にする。

Task 5はscale根拠の有無で内部を分ける。scale根拠が不足した場合もS/G、route flow、`physicalPixelLength`までは先に完了できるが、m距離表示が未完了ならTask 5およびPhase 6.1全体は完了扱いにしない。

## 受入条件

詳細は`docs/specs/2026-08-10-phase-06-1-field-ux-followups-design.md`を正本とする。

- pending outboxがあっても明示削除は可能で、破棄件数を本番confirmationで確認してから対象queueとデータを一緒に削除する。
- GAS/CSV previewやpreview反映など、Task 2で列挙した長時間処理に右下のloading/success/error表示がある。
...
- wide/tall mapをresetまたはarea変更しても正しいbase中央位置へ戻る。
- Galleryは開始時重く、購入閾値へ近づくほど表示追従が軽くなる一方、Phase 6と実質同じfinger travelで購入成立する。
