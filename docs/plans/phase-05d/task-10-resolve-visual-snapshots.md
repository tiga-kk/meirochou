...
すべてretry後も同じ種類の差分が再現しているため、単純なflaky snapshotとして扱わない。
...
- `tests/e2e/management.spec.ts-snapshots/`の対象画像
- `tests/e2e/webapp.spec.ts-snapshots/`の対象画像
- 各snapshotに対応する現行component / View / CSS
...
実際に原因と関係のあるファイルだけを変更する。5枚すべてのproduction fileを機械的に触らない。

## 各snapshotの判定

各snapshotを次のどちらかに必ず分類する。
...
現在の表示が既存仕様・意味論と一致しており、expected image側が古いことを履歴とDOM inspectionから説明できる。

対応:

- production codeをsnapshotに合わせて歪めない。
- CI固定環境の`npm run test:e2e:ci:update`で、そのsnapshotだけを更新する。
- 更新理由をcommit/進捗記録へ残す。
...
- Custom Element化・View分離によるwrapper差
- Phase 5D中にsnapshot自体が更新された履歴

### `outbox-recovery-panel.png`

高さが20px増えており、他の1px級差分より大きい。最優先で内容差を確認する。
...
- Phase 5D中のsnapshot更新後にさらに1px差が出た理由
...
1. CI固定containerでsnapshot対象E2Eを更新なしで実行し、fresh actual/diffを生成する。
2. 5枚を上記基準で個別に調査する。
3. `REGRESSION`と判定したものはproduction codeだけを修正し、そのsnapshotを再実行する。
4. `BASELINE_UPDATE`と判定したものはproduction codeを変更せず、CI固定containerで対象baselineを更新する。
5. snapshot更新時は意図した対象PNG以外が変わっていないことを`git diff --name-only`で確認する。
6. 5枚すべてが解消した後、management/webapp E2E全体を実行する。
7. full `npm run test:e2e:ci`を実行し、新しいsnapshot failureがないことを確認する。
...
対象を絞る場合も、Playwright container・project・viewportはCIと同一にする。

baseline更新が必要と判定した場合のみ:
...
- `--update-snapshots`を最初に実行して差分原因を消す。
- `maxDiffPixels`、`maxDiffPixelRatio`、thresholdを緩める。
- testをskipする。
- retry増加でGREEN扱いする。
- actual screenshotを手動コピーする。
- CIとは異なるOS/Playwrightの画像を正本にする。
- five snapshotsを一括更新してから事後的に理由を付ける。

## 受入条件

- 5枚それぞれについて`REGRESSION`または`BASELINE_UPDATE`の根拠を説明できる。
- regressionはproduction code修正で解消し、baselineを変更していない。
- baseline updateはCI固定環境から生成され、意図したsnapshotだけが変更されている。
...
production修正と単なるbaseline更新が独立してレビュー可能なら、無理に一commitへ混ぜない。
